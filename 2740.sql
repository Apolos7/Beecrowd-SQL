...
(select 'Podium: ' || league.team as name from league limit 3) 
union all
(select ('Demoted: ' || tab.team) from 
(select league.team from league order by league.position desc limit 2) as tab order by 1 asc);